...
// 生成新的钱包
// 从数据库中读取，如果不存在
func Rwq_NewWallets(nodeID string)(*Rwq_Wallets,error)  {
	wallets := Rwq_Wallets{}
	wallets.Rwq_Wallets = make(map[string]*Rwq_Wallet)

	err := wallets.Rwq_LoadFromFile(nodeID)
...
// 从数据库中读取钱包列表
func (ws *Rwq_Wallets)Rwq_LoadFromFile(nodeID string) error  {
...
// 将钱包存到数据库中
func (ws *Rwq_Wallets)Rwq_SaveToFile(nodeID string)  {
	walletFile := fmt.Sprintf(walletFile, nodeID)
...
// 打印所有钱包的余额
func (ws *Rwq_Wallets) Rwq_GetBalanceAll(nodeID string) map[string]int {
