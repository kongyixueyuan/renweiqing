...
// 新建区块链(包含创世区块)
func Rwq_CreateBlockchain(address string,nodeID string) *Rwq_Blockchain {
...
// 获取blockchain对象
func Rwq_NewBlockchain(nodeID string) *Rwq_Blockchain {
	dbFile := fmt.Sprintf(dbFile, nodeID)
	if !rwq_dbExists(dbFile) {
		log.Panic("区块链还未创建")
	}
...
// 生成新的区块(挖矿)
func (bc *Rwq_Blockchain) MineNewBlock(from []string, to []string, amount []string,nodeID string , mineNow bool) *Rwq_Block {
...
// 挖矿
func (bc *Rwq_Blockchain) Rwq_MineBlock(txs []*Rwq_Transaction) *Rwq_Block  {
	var lashHash []byte
	var lastHeight int
...
// 获取BestHeight
func (bc *Rwq_Blockchain) Rwq_GetBestHeight() int {
	var lastBlock Rwq_Block

	err := bc.rwq_db.View(func(tx *bolt.Tx) error {
		b := tx.Bucket([]byte(blocksBucket))
		lastHash := b.Get([]byte("l"))
		blockData := b.Get(lastHash)
		lastBlock = *Rwq_DeserializeBlock(blockData)
	...
	return lastBlock.Rwq_Height
}

// 获取所有区块的hash
func (bc *Rwq_Blockchain) Rwq_GetBlockHashes() [][]byte {
	var blocks [][]byte
	bci := bc.Rwq_Iterator()
	...
	for {
		...
		blocks = append(blocks, block.Rwq_Hash)
...
// 根据hash获取某个区块的内容
func (bc *Rwq_Blockchain) Rwq_GetBlock(blockHash []byte) (Rwq_Block, error) {
	var block Rwq_Block
...
// 将区块添加到链中
func (bc *Rwq_Blockchain) Rwq_AddBlock(block *Rwq_Block) {
	err := bc.rwq_db.Update(func(tx *bolt.Tx) error {
		b := tx.Bucket([]byte(blocksBucket))
		blockInDb := b.Get(block.Rwq_Hash)

		if blockInDb != nil {
			return nil
		}

		blockData := block.Rwq_Serialize()
		err := b.Put(block.Rwq_Hash, blockData)
		if err != nil {
			log.Panic(err)
		}
		...
		lastBlock := Rwq_DeserializeBlock(lastBlockData)

		if block.Rwq_Height > lastBlock.Rwq_Height {
			err = b.Put([]byte("l"), block.Rwq_Hash)
			if err != nil {
				log.Panic(err)
			}
			bc.rwq_tip = block.Rwq_Hash
		}
