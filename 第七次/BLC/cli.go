...
func (cli Rwq_CLI) Rwq_Run()  {
	...
	if createWalletCmd.Parsed() {
		cli.rwq_createWallet(nodeID)
	}

	if listAddressesCmd.Parsed() {
		cli.rwq_listAddrsss(nodeID)
	}
	...
	if createBlockchainCmd.Parsed() {
		...
		cli.rwq_createblockchain(*createBlockchainAddress,nodeID)
	}

	if printChainCmd.Parsed() {
		cli.rwq_printchain(nodeID)
	}
	...
	if getBalanceCmd.Parsed() {
		...
		cli.rwq_getBalance(*getBalanceAddress,nodeID)
	}
	if reindexUTXOCmd.Parsed() {
		cli.rwq_reindexUTXO(nodeID)
	}

	if printUTXOCmd.Parsed(){
		cli.rwq_printutxo(nodeID)
	}
	if getBalanceAllCmd.Parsed(){
		cli.rwq_getBalanceAll(nodeID)
	}
	if startNodeCmd.Parsed(){
		...
		cli.rwq_startNode(nodeID, *startNodeMiner)
	}
